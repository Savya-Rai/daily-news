# First Light Backlog

Ideas worth keeping out of GitHub issues for now.

## AI Briefing Layer

- Add a short Morning Snapshot paragraph under the masthead once AI summaries are introduced.
- Use AI to write sharper article summaries from source RSS metadata and article context, with RSS summaries as fallback.
- Generate editorial tags such as `Markets`, `Policy`, `Security`, `AI`, and `Geopolitics` if they prove useful for scanning.

## Reader Memory

- Add an optional Continue Reading affordance that remembers the last viewed section or expanded drawer on the same device.
- Avoid automatic jumping on page load; show a small `Continue from AI` style action instead.

## Personalization

- Add local, per-device section ordering preferences using `localStorage`.
- Keep default ordering for first-time visitors and for browsers/devices without saved preferences.
- Revisit sync only if the project ever adds accounts or a backend.

## Sharing

- Consider richer share affordances after story-level native share is tested in daily use.
- Do not add share controls for whole briefings or sections unless there is a clear need.

## Trust And Status

- Consider adding subtle success/freshness microcopy inside the masthead date plate, such as `Updated this morning`.
- Keep per-section warnings only for reduced coverage or feed failures so the reader-facing page stays clean.
